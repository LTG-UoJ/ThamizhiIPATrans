...
# Streamlit App UI
st.title("ThamizhiIPATrans")

# Text input section
tamil_text = st.text_area('Enter Tamil Text', height=200)
dialect_choice = st.selectbox("Select Dialect", ["Jaffna Sri Lankan", "Indian"])

if st.button('Convert Text to IPA'):
    if tamil_text:
        converted_text = tam2lat(tamil_text)
        ipa_result = ipa(converted_text, dialect_choice)
        st.markdown(f'**IPA Transcription ({dialect_choice}):** {ipa_result}')
    else:
